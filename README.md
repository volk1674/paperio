...
Класс [SimpleStrategy] содержит начальную версию стратегии, а класс [BestStrategy] улучшеная версия, которая из заняла 2 место в соревновании.   
...
с точки зрения производительности, так как приходилось по несколько раз обсчитывать 
...
Класс [MoveNode] отражает узел этого дерева. Дерево полностью формируются на старте стратегии. 
...
максимальный уровень риска, если бот был уже в пути на чужой или нетральной территории риск оценивается как разница
...
который использовался организаоторами в [Local Runner]. Предположу, что это было сделано намеренно, что бы не давать участникам
конкурса готовых решений.    
...
Например, бот соперника взявший замедление представлял опасность, так как при приближенном расчете стратегия пологала, 
что он должен был бы уже выйти из клетки, а он все еще был там. 
Для борьбы с этими проблемами стал рассчитать большее количество показателей для каждой клетки игрового поля (классы [AnalyticsBuilder] и [CellDetails]).
...
6.	Тик, на котором клетка сможет быть захвачена ботом соперника
