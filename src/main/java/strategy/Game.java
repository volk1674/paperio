...
public class Game {
	...
	static private void initCells() {
		...
		for (int i = 0; i < Game.sizeX; i++) {
			for (int j = 0; j < Game.sizeY; j++) {
				if (i > 0) {
					cells[i][j].neighborsMap.put(Direction.left, cells[i - 1][j]);
				}
				if (j > 0) {
					cells[i][j].neighborsMap.put(Direction.down, cells[i][j - 1]);
				}
				if (i < Game.sizeX - 1) {
					cells[i][j].neighborsMap.put(Direction.right, cells[i + 1][j]);
				}
				if (j < Game.sizeY - 1) {
					cells[i][j].neighborsMap.put(Direction.up, cells[i][j + 1]);
				}
			}
		}
	}
	...
	public static void buildTimeMatrix(int tick, int nb, int sb, Direction direction, int posX, int posY, int[][] result) {
		...
		if (result[cell.x][cell.y] > tick) {
			result[cell.x][cell.y] = tick;
			for (Direction nextDirection : cell.directions()) {
				...
				buildTimeMatrix(tick + width / speed, nb, sb, nextDirection, nextX, nextY, result);
			}
		}
	}
	...
	public static List<Cell> capture(PlayerState playerState) {
		...
		TerritoryBitMask territory = playerState.getTerritory();
		TerritoryBitMask lines = playerState.getLines();

		for (int i = 0; i < sizeX; i++) {
			for (int j = 0; j < sizeY; j++) {
				if (territory.isNotOccupied(cell(i, j)) && lines.isNotOccupied(cell(i, j))) {
					if (voids[i][j] == 0) {
						boolean captured = markVoids(playerState, zones.size() + 1, voids, cell(i, j));
						zones.add(captured);
					}
					if (zones.get(voids[i][j] - 1)) {
						result.add(cell(i, j));
					}
				} else if (lines.isOccupied(cell(i, j))) {
					result.add(cell(i, j));
				}
			}
		}
	...
	private static boolean markVoids(PlayerState playerState, int zone, int[][] voids, Cell cell) {
		voids[cell.x][cell.y] = zone;

		TerritoryBitMask territory = playerState.getTerritory();
		TerritoryBitMask lines = playerState.getLines();

		boolean result = !cell.border;
		for (Cell neighbor : cell.neighbors()) {
			if (voids[neighbor.x][neighbor.y] == 0 && lines.isNotOccupied(neighbor) && territory.isNotOccupied(neighbor))
