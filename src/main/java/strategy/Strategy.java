...
public class Strategy {
	...
	private List<MovePlan> bestPlans = new ArrayList<>();
	...
	public Direction calculate(int tick, Player me, List<Player> others, Map<Cell, Bonus> bonusMap) {
		...
		bestPlans.clear();
		List<MovePlan> bestPlans = estimatePlans(tick, me, generatePlans(directions), bonusMap);
	...
	private List<MovePlan> estimatePlans(int tick, Player me, List<MovePlan> plans, Map<Cell, Bonus> bonusMap) {
	...
	private boolean isInBestPlans(MovePlan plan, List<MovePlan> bestPlans) {
		ListIterator<MovePlan> bestPlansIterator = bestPlans.listIterator(bestPlans.size());
		while (bestPlansIterator.hasPrevious()) {
			MovePlan bestPlan = bestPlansIterator.previous();
				...
				for (Direction m2d : Direction.values()) {
					...
					for (int m2 = 1; m2 <= 10; m2 += 2) {
						...
						for (int m3 = 1; m3 <= 15; m3++) {
							plans.add(new MovePlan(new Move(m1d, m1), new Move(m2d, m2), new Move(m1d.opposite(), m3), new Move(m2d.opposite())));
						}
					}
				}
			}
	...
	public List<MovePlan> getBestPlans() {
		return bestPlans;
	}
